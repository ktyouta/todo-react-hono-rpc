...
export function TodoTrashList(props: PropsType) {
    ...
    const columns: TableProps<TodoTrashListReturnType['list'][number]>['columns'] = [
        ...
        { title: 'タイトル', field: 'title', className: 'w-[24%]', Cell: ({ entry }) => <span className="block truncate" title={entry.title}>{entry.title}</span> },
            ...
            <div className="flex-1">
                {taskData.list.length === 0 ? (
                    <div className="flex h-80 flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-[17px] text-gray-400 dark:text-gray-500">ゴミ箱にタスクがありません</p>
                    </div>
                ) : (
                    <>
                        {/* テーブル表示: lg 以上 */}

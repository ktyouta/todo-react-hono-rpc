...
export function TodoDeletedManagementDetailView(props: PropsType) {
        ...
        <div className="w-full min-h-full flex flex-col pb-4">
            ...
            <div className="mb-5">
                {task.parentId ? (
                    <Breadcrumb
                        items={[
                            { label: "削除タスク管理", href: paths.todoDeletedManagement.path },
                            ...task.ancestors.map((a) => ({
                                label: a.title,
                                href: paths.todoDeletedManagementDetail.getHref(a.id),
                            })),
                        ]}
                    />
                ) : (

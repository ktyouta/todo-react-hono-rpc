...
type PropsType = {
    ...
    setSearchCondition: (condition: TodoManagementSearchFilter) => void;
...
const baseColumns: TableProps<TaskManagementListReturnType['list'][number]>['columns'] = [
    ...
    { title: 'タイトル', field: 'title', className: 'w-[20%]', Cell: ({ entry }) => <span className="block truncate" title={entry.title}>{entry.title}</span> },
    ...
    { title: '優先度', field: 'priorityName', className: 'w-[8%] whitespace-nowrap', Cell: ({ entry }) => <Badge label={entry.priorityName} bgColor={getPriorityBadgeColor(entry.priorityId)} /> },
    {
...
export function TodoManagementList(props: PropsType) {

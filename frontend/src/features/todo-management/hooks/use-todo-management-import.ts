...
import { importTodoManagement, ImportTodoManagementResponseType } from "../api/import-todo-management";
import { todoManagementKeys } from "../api/query-key";
...
export function useTodoManagementImport() {
    ...
    // インポート結果
    const [result, setResult] = useState<ImportTodoManagementResponseType | null>(null);
    // 選択ファイル
    ...
    async function onImport(file: File) {
        setIsLoading(true);
        try {
            const data = await importTodoManagement(file);
            setResult(data);
            if (data.successCount > 0) {
                queryClient.invalidateQueries({ queryKey: todoManagementKeys.lists() });
            }
        } catch (error) {
...
export type UseTodoManagementImportReturn = ReturnType<typeof useTodoManagementImport>;

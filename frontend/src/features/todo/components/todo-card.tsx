...
export function TodoCard({ entry, onClick, onFavoriteToggle, isBulkMode = false, isSelected = false, onSelect }: PropsType) {
    return (
        ...
        >
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 text-xs relative">
                    ...
                    if (entry.statusId === STATUS_ID.COMPLETED) {
                        return (
                            <div>
                                ...
                                <span className="ml-1.5 text-gray-500 dark:text-gray-400">{dateStr}</span>
                            </div>
                        );
                    }

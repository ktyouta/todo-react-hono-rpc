...
export function TodoImportDialog({ isLoading, result, file, isDragging, onClose, onFileChange, onDrop, onDragOver, onDragLeave, onUpload }: PropsType) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
            <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col shadow-xl mx-4" onClick={handleContentClick}>
                    ...
                    {result ? (
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center gap-2 text-green-600">
                                ...
                                <p className="font-medium">{result.message}（{result.successCount}件更新）</p>
                            </div>
                            {result.errors.length > 0 && (
                                <div>
                                    <p className="text-sm text-gray-500 mb-2">エラー行（{result.errors.length}件）</p>
                            ...
                            {isLoading ? (
                                ...
                                <>
                                    <div
                                        ...
                                        className={`border-2 border-dashed rounded-lg p-12 flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors ${isDragging ? "border-blue-500 bg-blue-50" : "border-gray-300 hover:border-gray-400"
                                            }`}
                                    >
                                        <MdOutlineUploadFile className="size-12 text-gray-400" />
                                        <p className="text-sm text-gray-500">ここにCSVファイルをドロップ</p>
                                        <Button
                                            colorType="blue"
                                            sizeType="medium"

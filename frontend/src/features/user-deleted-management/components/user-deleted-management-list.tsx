...
export function UserDeletedManagementList(props: PropsType) {
    ...
    return (
        <div className="w-full min-h-full p-1 sm:p-5 flex flex-col">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3 text-right pr-2">全 {userData.total} 件</p>
            <div className="flex-1">
                {userData.list.length === 0 ? (
                    <div className="flex h-80 flex-col items-center justify-center gap-3">
                        <HiOutlineUserMinus className="size-12 text-gray-300 dark:text-gray-600" />
                        <p className="text-[17px] text-gray-400 dark:text-gray-500">削除済みユーザーがありません</p>
                    </div>
                ) : (
                    <>
                        {/* テーブル表示: lg 以上 */}
